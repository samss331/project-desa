...
const addPenduduk = async (
  ...
  try {
    const [results] = await db
      .promise()
      .query(
        "INSERT INTO PENDUDUK (nama, nik, alamat, tanggalLahir, jenisKelamin, agama, kepalaKeluarga) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [nama, nik, alamat, tanggalLahir, jenisKelamin, agama, kepalaKeluarga]
      );
    return (
      results.insertId,
      nama,
...
const updateDataPenduduk = async (
  ...
  try {
    const [results] = await db
      .promise()
      .query(
        "UPDATE PENDUDUK SET nama = ?, nik = ?, alamat = ?, tanggalLahir = ?, jenisKelamin = ?, agama = ?, kepalaKeluarga = ? where nik =?",
        [
          nama,
          newNik,
          alamat,
...
const getTotalKepalaKeluarga = async () => {
  try {
    const [results] = await db
      .promise()
      .query("SELECT COUNT(*) AS total FROM PENDUDUK WHERE kepalaKeluarga = 1");
    return results[0].total;
  } catch (error) {
    throw error;
  }
