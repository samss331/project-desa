...
import { kepala_keluargaDTO } from "../dto/dto.js";

const getAllKepalaKeluarga = async () => {
  try {
    const results = await kepalaKeluargaRepo.getAllKepalaKeluarga();
    return results.map((kk) => new kepala_keluargaDTO(kk.id, kk.nama, kk.nik));
  } catch (error) {
...
const getKepalaKeluargaById = async (id) => {
  try {
    ...
    return new kepala_keluargaDTO(result.id, result.nama, result.nik);
  } catch (error) {
...
const updateKepalaKeluarga = async (id, nama, nik) => {
  ...
  try {
    const updated = await kepalaKeluargaRepo.updateKepalaKeluarga(
      id,
      nama,
      nik
    );
    ...
    return new kepala_keluargaDTO(id, nama, nik);
  } catch (error) {
