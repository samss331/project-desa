...
const getAllPenduduk = async () => {
    try {
        ...
        return results.map(p => new PendudukDTO(p.id, p.nama, p.nik, p.alamat, p.tanggalLahir, p.jenisKelamin, p.agama, p.kepalaKeluarga));
    } catch (error) {
...
const getPendudukByNik = async (nik) => {
    try {
        ...
        return new PendudukDTO(result.id, result.nama, result.nik, result.alamat, result.tanggalLahir, result.jenisKelamin, result.agama, result.kepalaKeluarga);
    } catch (error) {
        console.log(error)
...
const addPenduduk = async (nama, nik, alamat, tanggalLahir, jenisKelamin, agama, kepalaKeluarga) => {
    if (!nama || !nik || !alamat || !tanggalLahir || !jenisKelamin || !agama || !kepalaKeluarga) {
        throw new Error("Semua data wajib diisi!");
    }
    try {
        const result = await pendudukRepo.addPenduduk(nama, nik, alamat, tanggalLahir, jenisKelamin, agama, kepalaKeluarga);
        return new PendudukDTO(result.id, result.nama, result.nik, result.alamat, result.tanggalLahir, result.jenisKelamin, result.agama, result.kepalaKeluarga);;
    } catch (error) {
...
const updateDataPenduduk = async (oldNik, nama, newNik, alamat, tanggalLahir, jenisKelamin, agama, kepalaKeluarga) => {
    ...
    await pendudukRepo.updateDataPenduduk(oldNik, nama, newNik, alamat, tanggalLahir, jenisKelamin, agama, kepalaKeluarga);
    return { success: true, message: "Data berhasil diperbarui", nama, newNik, alamat, tanggalLahir, jenisKelamin, agama, kepalaKeluarga };
};
...
export default { getAllPenduduk, getPendudukByNik, 
    addPenduduk, updateDataPenduduk, deleteDataPenduduk, 
    getPendudukByUmur, getTotalKepalaKeluarga, getTotalLakiLaki, 
    getTotalPerempuan, getTotalPenduduk, getPendudukByAgama};
